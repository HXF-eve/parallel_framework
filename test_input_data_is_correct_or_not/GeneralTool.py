...
class GeneralTool:
	...
	def Get_a_random_element_from_a_list( self, input_list ):
		...
		## --- normal routine:
		ret_rdm_element = random.choice(input_list)
		return ret_rdm_element## EoF:~
	# ================================================
	def Get_a_random_key_of_a_dict( self, input_dict ):
		rdm_key = random.choice(list(input_dict.keys()))
		## --- 3 return this key
		return rdm_key##:~
	# ================================================
	...
	def Get_a_number_of_random_samples_from_a_list(self, input_list, num_sample):
		...
		if num_sample > size_of_list:
			print('Warning: Get_a_number_of_random_samples_from_a_list, num_sample > len(input_list).\n')#
			num_sample = size_of_list#
		## --- normal routine:
	...
	def Get_the_key_with_max_value_of_a_dict( self, dct_input ):
		return max(dct_input.items(), key=operator.itemgetter(1))[0]## EoF:~
	def Get_the_key_with_min_value_of_a_dict( self, dct_input ):
		if not dct_input:
			return 'Non'#:~
		return min(dct_input.items(), key=operator.itemgetter(1))[0]## EoF:~
	# ================================================
	...
	def Get_all_combinations_from_a_list( self, lst_ele, length ):
		...
		return ret_list## EoF:~
	# ================================================
	def Is_all_elements_in_list_are_non_varying( self, vec ):
		## --
		for idx in range(len(vec)):
			ele = vec[idx]
			if idx+1 in range(len(vec)):
				ele_next = vec[idx+1]
				if ele != ele_next:
					return False##
		## --
		return True###:~		
	def Is_all_elements_in_list_are_varying_nonNegative( self, vec ):
		## --
		bool_nonVarying = self.Is_all_elements_in_list_are_non_varying(vec)#
		if bool_nonVarying:
			return False
		else:
			for ele in vec:
			...
			return True###:~		
	# ================================================
	def Is_all_elements_in_list_are_varying_nonPositive( self, vec ):
		## --
		bool_nonVarying = self.Is_all_elements_in_list_are_non_varying(vec)#
		if bool_nonVarying:
			return False
			...
			return True###:~		
	# ================================================
	def Is_all_elements_in_list_are_varying_from_negative_to_positive( self, vec ):
		## --
		bool_nonVarying = self.Is_all_elements_in_list_are_non_varying(vec)#
		if bool_nonVarying:
			return False
		else:### elements are varying from negative to positive.
			item_first = vec[0]
			item_last = vec[len(vec)-1]
			if item_first<0 and item_last>0 and item_first*item_last<0:
				return True##
			## --
			return False###:~
	# ================================================